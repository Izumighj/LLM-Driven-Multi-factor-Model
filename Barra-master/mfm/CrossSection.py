...
if __name__ == '__main__':
    ...
    data = pd.read_csv('Barra-master/data_new/barra_data_csi.csv')
    base_data = data[['date','stocknames','capital','ret']]

    style_factors_list = list(data.columns[5:])
    style_factors = data[style_factors_list]

    industry_factors = data[['industry']]
    ...
    # 打印模型计算出的因子收益率
    # 顺序是：截距项, 4个行业因子, 6个风格因子
    print("\n计算出的因子收益率 (factor_ret):")
    print(factor_ret)

    # 打印预设的“真实”因子收益率，用于对比
    # 顺序是：4个行业因子, 6个风格因子 (注意：这里没有截距项)
    #print("\n预设的真实因子收益率 (sensitivity):")
    #print(sensitivity)
