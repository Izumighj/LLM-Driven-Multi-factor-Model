...
def main():
    ...
    barra_df = pd.merge(barra_df, sw_industry_data[['ts_code', 'l1_code']], on='ts_code', how='left')
    
    # 4.2 重命名列
    barra_df.rename(columns=config.COLUMN_RENAME_MAP, inplace=True)
    
    # 4.3 按指定顺序选择最终列
    final_barra_df = barra_df[config.BARRA_OUTPUT_COLUMNS]
    ...
    # 5.2 生成并保存行业信息
    ind_info = pd.merge(stk_data[['ts_code']].drop_duplicates(), sw_industry_data[['ts_code','l1_code','l1_name','in_date']], on='ts_code', how='left')
